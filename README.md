...
| `get_precedent` | 판례 조회 | 검증된 사건번호·요지(키워드/주제 검색) — **192건, 실재 판례만** + 사건번호별 casenote 딥링크 |
...
- **판례 192건**(고유 181, 판례 보유 주제 150/168, 2020년 이후 70건). 각 사건번호는 law.go.kr·casenote.kr 판결문 실열람 검증분만 수록 — **미검증·없는 판례는 지어내지 않음**
...
- [x] 검증된 판례 DB(`get_precedent`) — 192건, law.go.kr/casenote 실열람 검증
