...
**49개 분야 · 203개 주제** — 노동·주택/상가임대차·돈거래/사기·소비자·교통사고·민사/형사 절차·가정폭력·성범죄·스토킹·디지털성범죄·명예훼손·가사/상속·채무조정·금융사기·학교폭력·산업재해·행정·의료·조세·계약·부동산·출입국·보험·지식재산·아동/노인학대·고용보험·공동주택·통신/개인정보·군·선거·환경·반려동물·**외국인/이주민·청소년/미성년·장애인·북한이탈주민·플랫폼/특수고용·국가유공자/보훈·복지/취약가구·농어업인·노인/고령·정신건강·범죄피해자·자살예방/유족·재난/안전(취약계층·위기)** 등.
...
npm run typecheck
npm test             # vitest 28개 (계산 결정성·데이터 불변식·인용검증 회귀·통합)
...
| `get_form_template` | 표준 서식 | 진정서·내용증명·고소장·지급명령·가압류 등 + **무료지원·구제 신청서 14종**(소송구조·재산관계진술서·범죄피해구조금·간이대지급금·분쟁조정·디성센터 삭제지원·양육비이행·전세사기 결정·채무조정·**사회보장급여·자립수당·국가유공자 등록·산재 요양급여·외국인 사업장변경**) 빈칸 채움 골격 + 작성요령·**공식 양식 받는 곳** + **`.txt` 파일 다운로드 링크**(드라이브·카톡 '나에게 보내기'로 공유) (자동작성 아님) |
| `get_precedent` | 판례 조회 | 검증된 사건번호·요지(키워드/주제 검색) — **196건, 실재 판례만** + 사건번호별 casenote 딥링크 |
...
- **203개 주제** / 49개 분야 — `src/data/*.ts`(분야별 분리) → `index.ts` 병합
- **판례 197건**(고유 186, 판례 보유 주제 155/203, 2020년 이후 72건). 각 사건번호는 law.go.kr·casenote.kr 판결문 실열람 검증분만 수록 — **미검증·없는 판례는 지어내지 않음**
- **법률용어 125개**(`src/data/glossary.ts`, 9개 분류 — easylaw.go.kr·law.go.kr·대법원·헌재 본문 검증, 형제자매 유류분 위헌 등 최신 반영)
- 표준서식 77종(무료지원·구제 신청서 14종 포함) · 법령 요지 218건 · 시점법 타임라인 12건 · 인용 유효성 플래그 14건
...
- [x] 생활법률 전 분야 확장 — 49개 분야 203개 주제(외국인노동자·이주여성·청소년·장애인·북한이탈주민·플랫폼특수고용·국가유공자·기초생활·한부모·농어업인·노인·정신건강·범죄피해자·자살예방/유족·재난안전 등 취약계층·위기 35주제 포함)
- [x] 검증된 판례 DB(`get_precedent`) — 196건, law.go.kr/casenote 실열람 검증
- [x] 법령 공식 deep-link grounding (`get_statute`)
- [x] 자연어 검색·트리아지(`search_topics`·`triage`)
- [x] 인용 검증·시점법(`verify_citation`·`law_updates`) — 환각 차단
- [x] 무료지원·구제 신청서 빈칸 채움 골격 14종(`src/data/apply_forms.ts`) — 소송구조·구조금·대지급금·분쟁조정·전세사기·채무조정 + 사회보장급여·자립수당·국가유공자 등록·산재 요양급여·외국인 사업장변경. declaw 경계 유지, 공식양식 출처·작성요령 동봉
...
- [x] 자동 테스트(`test/`, vitest 28개) — 계산 결정성·데이터 정합성 불변식·코드리뷰가 잡은 버그 회귀(인용 거짓확인·불가능 날짜·∞ 입력·용어 오매칭)·통합(도구 규격·면책·다운로드)
