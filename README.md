...
- **판례 194건**(고유 184, 판례 보유 주제 154/203, 2020년 이후 72건). 각 사건번호는 law.go.kr·casenote.kr 판결문 실열람 검증분만 수록 — **미검증·없는 판례는 지어내지 않음**. ★전 사건번호를 casenote·law.go.kr로 **전수 재검증**해 미실존 3건 제거·오기 2건 정정(자동 회귀 테스트로 재발 차단)
...
- [x] 자동 테스트(`test/`, vitest 62개) — 계산 결정성·데이터 정합성 불변식·할루시네이션 재발 가드(미실존 사건번호·비표준 조문)·코드리뷰가 잡은 버그 회귀·통합(도구 규격·면책·다운로드)
