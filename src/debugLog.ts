...
//  3) 매칭 실패여도 자유 텍스트는 남기지 않는다. 카카오 개발가이드 §5.3은
//     사용자 프롬프트 획득을 금지하므로, 마스킹·메모리 보관이어도 원문을 담지 않는 쪽이 맞다.
//  4) 기본 꺼짐(DEBUG_LOG=on일 때만 수집). 켜도 도구명·인자 키·소요시간·성공여부만 남긴다.
...
export function logCall(entry: {
  ...
  const full: LogEntry = {
    ...
    ms: entry.ms,
    ok: entry.ok,
    // 예외 메시지에 입력값이 섞어 있을 수 있으므로 원문을 보관하지 않는다.
    error: entry.error ? "도구 호출 실패" : undefined,
    flag: entry.flag,
  };
  ring.push(full);
