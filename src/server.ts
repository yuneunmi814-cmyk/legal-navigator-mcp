...
const SERVER_INSTRUCTIONS =
  "이 서버는 한국 생활법률 53개 분야 211개 주제(노동·임대차·상가·돈거래/사기·소비자·교통사고·민사/형사 절차·가정폭력·성범죄·스토킹·가사/상속·채무조정·금융사기·산재·행정·의료·조세·계약·부동산·출입국·보험·지식재산·학대·고용보험·통신/개인정보·군·선거·환경·반려동물·외국인/이주민·청소년/미성년·장애인(등록·활동지원)·북한이탈주민·플랫폼/특수고용·국가유공자/보훈·복지/취약가구·농어업인·노인/고령·정신건강·범죄피해자·자살예방/유족·재난/안전·소상공인/폐업재기·출소자/갱생보호·위기임신/보호출산·공적연금/사회보험(국민연금·건강보험 급여) 등)에 대한 " +
...
export function createServer(baseUrl?: string): McpServer {
  ...
  server.registerTool(
    ...
    async ({ query }) => {
      ...
      if (!ranked.length) {
        return { content: [{ type: "text", text: withDisclaimer(`'${query}'에 맞는 주제를 바로 찾지 못했습니다. list_topics로 전체 목록(53개 분야)을 확인하거나, 더 구체적인 표현으로 다시 검색해 주세요.`) }] };
      }
