...
const SERVER_INSTRUCTIONS =
  ...
  "사용자가 번호로 답하면 그 뜻으로 받아들이고 되묻지 마세요. 질문은 **최대 3번**이며, 그 뒤에는 더 묻지 말고 답을 확정합니다. " +
...
function 진단보조지침(topic: string): string {
  ...
  l.push(
    ...
    "- 최대 3번까지만 묻는다. 그 뒤에는 더 묻지 말고 답을 확정한다.",
    "- 사용자가 번호로 답하면 그 뜻으로 받아들이고 되묻지 않는다.",
  );
  if (forms.length) {
    l.push(
      `- 이 주제의 표준 서식: ${forms.map((f) => `\`${f}\``).join(" · ")}`,
      "- **답을 확정한 직후**, 또는 사용자가 서식·양식·신청서·진정서·내용증명을 요구하거나" +
        " 대화가 '어떻게 접수하느냐'로 넘어가면 위 키로 **get_form_template을 호출**한다." +
        " 서식 카드가 그때 화면에 뜬다. 서식 본문을 직접 지어내지 말 것.",
    );
  }
  l.push(
...
<!-- 진행 지침: search_topics를 query 없이 호출하면 전체 분야 목록을 받는다. 도구 이름은 노출하지 말 것. -->`) }] };
      ...
      const parts = [
        ...
        `⏰ **${p.기한}**`,
        ``,
        `**지금 먼저** — ${p.단계[0].replace(/^\d+\)\s*/, "")}`,
      ];
      // 접수처는 '다음 행동'이라 빼지 않는다(8/9 결정: 문제 상황 → 관련 법 + 제출 방법).
      // 다만 섹션 제목을 달아 늘리지 않고 한 줄로 접는다.
      // 증거 확보는 기한 다음으로 중요하다. 섹션을 없애면서 통째로 빠졌었는데,
      // 259개 중 187개는 1단계가 증거 얘기가 아니라 그 주제들은 증거 정보가 사라졌다(2026-08-22 확인).
      // 목록으로 늘리지 말고 한 줄로 잇는다.
      const evid = (c?.증거 ?? []).slice(0, 3).join(" · ");
      if (evid) parts.push(``, `📎 **챙길 것** — ${evid}`);
      parts.push(``, `📮 **접수** — ${p.온라인접수}`, ``, `⚖️ ${p.근거법.join(" · ")}`);
