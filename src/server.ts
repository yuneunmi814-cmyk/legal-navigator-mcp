...
export const SERVER_INSTRUCTIONS =
  // 이 첫 문장이 호스트 LLM이 "이 서버를 부를까"를 판단하는 지점이다. 여기에 없는 영역은
  // 아무리 주제가 있어도 신호가 가지 않는다 — 실제로 "경찰이 응급입원 시켰어"에서 우리 툴이
  // 호출되지 않았고, 분야 목록에 경찰·입원·체포가 통째로 빠져 있었다(2026-09-01 실기기 확인).
  // 목록을 늘릴 때는 주제를 만든 뒤 반드시 여기도 같이 고칠 것.
  `한국 생활법률 ${TOPIC_KEYS.length}개 주제의 절차·기한·표준 서식·금액 계산·법령/판례를 제공합니다. ` +
  "다루는 영역: 노동(임금체불·부당해고·5인 미만 사업장·주휴수당·프리랜서 근로자성)·주택/상가 임대차·전세사기·돈거래/사기·보이스피싱·소비자·교통사고(자동차·자전거·전동킥보드)·민사절차(지급명령·소액소송·가압류·강제집행)·형사절차(고소·불기소 항고·재정신청·약식명령)·" +
  "경찰의 조치를 다투는 절차(현행범 체포의 적법성·임의동행·불심검문·보호조치·압수수색과 휴대전화 임의제출·형사보상·국가배상·공무원 불친절 민원)·본인 뜻과 다른 정신의료기관 입원(응급입원·보호입원·행정입원)과 정신질환자 권리·" +
  "가정폭력/성폭력/스토킹/디지털성범죄·명예훼손·허위조작정보·학교폭력·가사/상속·채무조정/개인회생·산업재해·행정심판/소송·의료분쟁·조세·부동산매매·출입국/외국인·보험·지식재산·아동/노인학대·고용보험·군·병역·환경·반려동물·장애인·복지/급여 등 57개 분야. " +
  ...
  // declaw의 "공란을 채우는 수준까지만"이 브레이크로 읽혔다 — 실기기에서 이름·금액을
// 다 말했는데도 "제가 임의로 채워서 제공하기보다는"이라며 초안을 거절했다(2026-09-02).
// 금지선은 그대로 두되, 채우는 것 자체는 **하라고** 뒤집어 적는다.
  "declaw: 개별 법률 자문이 아닙니다. 사건의 결론(승소·유무죄)을 단정하지 마세요. "
  + "**서식의 빈칸을 사용자가 말한 값으로 채우는 것은 이 서비스가 하기로 되어 있는 일입니다** — "
  + "사용자가 말한 값을 옮겨 적는 것은 창작이 아니므로 거절하지 마세요. 다만 사용자가 말하지 않은 사실을 지어내거나 "
  + "법적 주장·의견을 대신 써 주지는 마세요. 없는 판례·법령은 지어내지 말고, 중대·기한임박 사안은 "
  + "변호사·대한법률구조공단(132) 상담을 권하세요.";
...
function rankTopics(query: string): string[] {
  ...
  // 5인 미만 사업장은 근로기준법 제23조①·제28조가 적용되지 않아 노동위 구제신청 자체가 안 된다
  // (시행령 별표 1 전수 확인). 그런데 "5인 미만인데 부당해고"는 두 주제에 다 걸리고,
  // '부당해고'라는 낱말이 제목에 정확히 있어 부당해고가 1등이 된다 — 그러면 갈 수 없는 곳으로
  // 안내하게 된다(2026-08-31 로컬에서 재현). 규모 신호가 있으면 규모 주제를 앞에 세운다.
  // 2026-09-01 보강 — 규모를 말하는 방식이 사람마다 너무 다르다. 아래 넷은 전부 실패했다:
  //   "직원이 나 포함 세 명인데 갑자기 나오지 말래요"  → 못 찾음
  //   "작은 가게라 직원이 저 혼자인데 잘렸어요"        → 부당해고 구제(=각하 경로)
  //   "사장님이랑 저밖에 없는 회사인데 그만두래요"      → 부당해고 구제
  //   "직원 3명인 회사에서 해고됐어요"                 → 부당해고 구제
  // 원인 둘. ①'직원이' 바로 뒤에 인원이 와야 잡히는 정규식이라 "나 포함 세 명"을 놓쳤고,
  // '저 혼자'·'저밖에 없는'은 아예 없었다. ②규모 주제가 후보 점수에 아예 안 잡힌 발화에서는
  // 기존 규칙이 '이미 있는 점수를 올리는' 방식이라 아무 일도 하지 않았다.
  // 그래서 규모 신호가 해고·수당 맥락과 함께 오면 후보에 없더라도 밀어 넣는다.
  const 소규모신호 =
    /5인\s*미만|5명\s*미만|다섯\s*명\s*미만|4명\s*이하|네\s*명\s*이하|영세\s*사업장|상시\s*근로자\s*[1-4]\s*명/.test(Q) ||
    /직원[^.?!]{0,12}?(나\s*혼자|저\s*혼자|한\s*명|두\s*명|세\s*명|네\s*명|[1-4]\s*명|[1-4]\s*명뿐)/.test(Q) ||
    /(나|저|본인)\s*(밖에|말고는)\s*(직원이?\s*)?없/.test(Q) ||
    /(사장|사장님|대표)[^.?!]{0,10}?(나|저)\s*(밖에|둘|뿐)/.test(Q) ||
    /(나|저|본인)\s*(포함|합쳐|빼고)[^.?!]{0,6}?(한|두|세|네|[1-4])\s*명/.test(Q);
  // 규모 주제가 답이 되어야 하는 맥락 — 해고(노동위 구제가 막힘)와
  // 가산수당·연차·휴업수당(별표 1에 없어 아예 적용되지 않음).
  const 해고맥락 = /해고|짤렸|잘렸|자르|나가라|나오지\s*말|그만두라|그만두래|그만두라고|권고사직|퇴사하라/.test(Q);
  const 수당맥락 = /야근수당|연장수당|가산수당|초과근무수당|특근수당|휴일수당|연차|휴업수당/.test(Q);
  if (소규모신호 && (score.has("오인미만사업장") || 해고맥락 || 수당맥락)) {
    const top = score.size ? Math.max(...score.values()) : 0;
    score.set("오인미만사업장", top + 3);
    strong.set("오인미만사업장", (strong.get("오인미만사업장") ?? 0) + 1);
  }
  // 프리랜서·개인사업자로 계약한 사람이 퇴직금·임금을 물으면 퇴직금 주제가 이겼다
  // ("프리랜서인데 4대보험도 안 되고 퇴직금도 못 받나요" → 퇴직금 미지급, 2026-09-01 확인).
  // 순서가 뒤집힌 안내다 — 근로자로 인정받는 것이 먼저이고, 그걸 건너뛰고 노동청에
  // 퇴직금 진정을 넣으면 "당신은 근로자가 아니다"에서 막힌다. 5인 미만과 같은 종류의 사고다.
  const 근로자성신호 = /프리랜서|위탁\s*계약|위임\s*계약|개인사업자|3\.3\s*(%|프로|퍼센트)|사업소득으로/.test(Q);
  const 근로조건청구 = /퇴직금|임금|월급|급여|해고|잘렸|연차|4대\s*보험|주휴/.test(Q);
  if (근로자성신호 && 근로조건청구 && score.size) {
    const top = Math.max(...score.values());
    score.set("프리랜서근로자성", top + 2);
    strong.set("프리랜서근로자성", (strong.get("프리랜서근로자성") ?? 0) + 1);
  }
...
function 작성지침(f: { 본문: string }): string {
  ...
  return [
    ...
    // 실기기에서 모델이 "제가 임의로 채워서 제공하기보다는"이라며 초안을 거절하고
    // 값을 목록으로만 나열했다(2026-09-02). 옮겨 적기는 창작이 아니라는 것을 못 박는다.
    "- ⛔ **초안 작성을 거절하지 마세요.** '임의로 채우기보다는' '직접 입력하시는 편이' 같은 말로 넘기지 말 것. "
      + "사용자가 말한 값을 정해진 칸에 옮겨 적는 것은 창작이 아니라 이 도구가 하기로 되어 있는 일입니다. "
      + "값을 목록으로 나열하는 것으로 대신하지 말고, **서식 모양 그대로** 채워서 보여주세요.",
    "- 보여줄 모양: 서식 본문을 코드블록에 그대로 넣고, 사용자가 말한 값만 해당 칸에 바꿔 넣습니다. "
      + "모르는 칸은 [빈칸] 표시를 남겨 두면 됩니다.",
    // 카드의 '빈칸 바로 채우기' 버튼과 '직접 입력하세요' 안내를 보고 모델이 채팅에서의
    // 초안 작성을 통째로 미뤘다(2026-09-02 실기기). 둘의 용도를 갈라 준다.
    "- ⛔ 카드에 있는 '빈칸 바로 채우기' 버튼과 '직접 입력하세요' 안내를 이유로 초안을 미루지 마세요. "
      + "그 버튼은 사용자가 **인쇄·제출용 문서**를 만들 때 쓰는 것이고, 화면의 '직접 입력' 안내는 "
      + "**주민등록번호·계좌번호에만** 해당합니다. 이름·금액·기간처럼 사용자가 이미 말한 값은 "
      + "지금 이 채팅에서 채워 보여주는 것이 맞습니다.",
    "- 값이 일부만 있어도 그 상태로 먼저 보여주세요. \"나머지 정보가 없어서 빈칸으로 두는 게 안전합니다\"라며 "
      + "초안을 건너뛰지 말 것 — 아는 값은 채우고 모르는 칸만 [빈칸]으로 남긴 초안이 정답입니다.",
