...
describe("데이터 정합성", () => {
  ...
  // 2026-09-01 서식 리뷰 — 눈으로 45종을 읽고 찾은 것들을 회귀로 고정한다.
  it("서식 리뷰 9/1 — 각하되는 안내와 빠진 제출처를 되살리지 않는다", () => {
    // 상시 근로자 수 칸은 있었는데 4명 이하면 신청 자체가 각하된다는 경고가 없었다
    const 부당해고 = FORMS["부당해고구제신청서"];
    expect(부당해고.작성요령.join(" ")).toContain("각하");
    expect(부당해고.본문).toContain("지방노동위원회");
    // 진정서인데 어디에 내는지가 없었다
    expect(FORMS["임금체불진정서"].본문).toContain("지방고용노동청");
    // 2023.4.18 개정 — 임대인 송달 전에도 집행되고, 고지받은 날부터 2주가 지나면 집행 못 한다
    const 임차권 = FORMS["임차권등기명령신청서"].작성요령.join(" ");
    expect(임차권).toContain("송달받기 전에도");
    expect(임차권).toContain("2주");
    // 신고서인데 제출처가 없었다
    expect(FORMS["스토킹피해신고서"].작성요령.join(" ")).toContain("경찰서");
