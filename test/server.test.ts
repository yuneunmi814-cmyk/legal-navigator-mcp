...
describe("가족·지인 간 차용증 없는 대여('떼인 돈')", () => {
  ...
  // 계약·용역·동업 미이행 계열 — 2026-09-04 카카오 실사용 테스트에서 통째로 비어 있던 영역.
  // 실제 상황(공동수급자가 잠수 → 내가 대신 이행 → 외주비 실비 청구)을 넣었더니
  // 라이브 1순위가 **재판이혼**이었다. 원인은 rankTopics가 공백으로만 쪼갠 어절을
  // 적용대상 문장에 substring 매칭하는 것 — "상대방이"·"자기"·"않고" 같은 조사 붙은
  // 기능어가 +2점 근거로 잡히고, weak>=2 게이트를 그 둘만으로 통과한다.
  it("계약·용역·동업 미이행 발화가 무관한 분야로 새지 않는다 (9/4 실사용)", async () => {
    const 금지 = /재판이혼|협의이혼|성폭력|가정폭력|스토킹|소년보호|공시송달|퇴직금미지급|전세보증금반환|프리랜서근로자성/;
    const 사례: [string, RegExp][] = [
      // ── 상대 미이행(계약 위반)
      ["계약 상대방이 일을 안 해서 손해를 봤다", /채무불이행|계약해제해지|손해배상청구/],
      ["동업자가 자기 몫 일을 안 하고 잠수타서 내가 대신 처리했다", /채무불이행|계약해제해지|손해배상청구|부당이득반환청구/],
      ["공동수급자가 마감을 안 지켜서 내가 대신 외주비를 썼다", /채무불이행|계약해제해지|손해배상청구|부당이득반환청구/],
      ["외주업체가 납기를 못 맞춰서 손해가 났어요", /채무불이행|계약해제해지|손해배상청구/],
      ["같이 일하기로 한 사람이 연락이 두절인데 어떻게 하나요", /채무불이행|계약해제해지|손해배상청구|민사조정/],
      ["용역 계약했는데 상대가 약속한 작업을 안 끝냈어요", /채무불이행|계약해제해지|손해배상청구/],
      // ── 일한 대금 미수
      ["일해준 대금을 못 받고 있어요", /물품용역대금미수금|채무불이행/],
      ["프리랜서로 작업 납품했는데 대금을 안 줘요", /물품용역대금미수금|채무불이행/],
      ["용역비를 3개월째 안 주는 거래처", /물품용역대금미수금|채무불이행/],
      ["디자인 외주를 끝냈는데 잔금을 안 줍니다", /물품용역대금미수금|채무불이행/],
      // ── 대신 지출한 비용 회수(구상)
      ["상대방이 할 일을 내가 대신 하고 쓴 비용을 돌려받고 싶어요", /채무불이행|부당이득반환청구|손해배상청구/],
      ["동업자 대신 낸 외주비 16만원을 청구하려면 어떻게 하나요", /채무불이행|부당이득반환청구|손해배상청구|물품용역대금미수금/],
      ["제가 대신 부담한 비용을 상대에게 돌려받을 수 있나요", /채무불이행|부당이득반환청구|손해배상청구/],
      // ── 받아내는 절차
      ["16만원 받으려고 소송하는 게 나을까요", /지급명령|소액사건소송|민사조정|채무불이행/],
      ["적은 금액인데 법적으로 받아낼 방법이 있나요", /지급명령|소액사건소송|민사조정|채무불이행/],
      ["내용증명을 보냈는데 답이 없으면 다음은 뭔가요", /지급명령|소액사건소송|민사조정|채무불이행|강제집행/],
      ["돈 받아내는 절차 중에 제일 빠른 게 뭐예요", /지급명령|소액사건소송|민사조정/],
      // ── 실제 상황 전문(긴 자연어 서술). 라이브에서 재판이혼이 1위였던 그 발화.
      [
        "저와 다른 프리랜서가 고객사와 5대5로 대금을 받기로 공동계약했는데, 상대가 자기 몫을 이행하지 않고 잠수를 타서 제가 대신 다 처리했고 외주비 16만원을 썼습니다. 실비를 청구했는데 답이 없습니다",
        /채무불이행|계약해제해지|손해배상청구|부당이득반환청구|물품용역대금미수금|지급명령|소액사건소송|민사조정/,
      ],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (금지.test(top) || !기대.test(top)) 실패.push(`  ✗ ${top.padEnd(22)} ← ${q.slice(0, 46)}`);
    }
    expect(실패, `
${사례.length}개 중 ${실패.length}개 실패
${실패.join("\n")}
`).toEqual([]);
  });
  // 채무·전세 카페(ccrs5500·cafe20135031) 실사용 제목 202개를 라우팅에 돌려 나온 빈틈.
  // 공통 원인은 실무에서 쓰는 말과 법률 용어가 다른 것이다 — 명도/인도, 원상복구/원상회복,
  // 압류방어/압류방지. 주제는 전부 이미 있는데 거기로 가는 말이 없어 0건이었다.
  // 202개 기준 79% → 87% (0건 43 → 27). 남은 27개는 대부분 후기·정보공유 글이라 0건이 맞다.
  it("카페 실사용 제목 — 실무어로도 주제에 닿는다 (9/7)", async () => {
    const 사례: [string, RegExp][] = [
      ["임차권등기 이후 지연이자", /임차권등기명령/],
      ["도와주세요 터무니없는 원상복구비용", /원상회복범위|임차인관리의무/],
      ["보증보험기간 만료됐다네요 어떻게 해야 할까요", /전세보증보험이행청구/],
      ["중도해지합의서 및 보증이행절차 문의드립니다", /전세보증보험이행청구/],
      ["최종 이행 거절 받았습니다", /전세보증보험이행청구/],
      ["명도통보 내용증명", /건물인도청구/],
      ["명도일이 걱정이에요", /건물인도청구/],
      ["장기연체 준비하는 분들에게 압류방어용 통장개설 방법 공유합니다", /압류방지통장/],
      ["대학생 청년 신용회복지원", /신용회복위원회채무조정/],
      ["경매 셀프 낙찰 자금 마련(대출)에 대해 질문드립니다", /임차인경매대응/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!기대.test(top)) 실패.push(`  ✗ ${top.padEnd(22)} ← ${q.slice(0, 46)}`);
    }
    expect(실패, `\n${사례.length}개 중 ${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // 위 동의어는 '명도'·'압류방어'처럼 넓은 말이라 게시판 잡담까지 끌고 올 수 있다.
  // 넓히는 변경마다 이쪽을 함께 본다(스토킹 네거티브와 같은 취지).
  // 제외한 셋: "서울 서부 명도 완료 타임라인"·"2014년 2분기 신용회복지원 실적"·
  // "드디어 오늘 워크아웃 체결했습니다 대박"은 이제 매칭되지만,
  // 상담창에 칠 문장이 아니라 실해가 없다. 이걸 0건으로 만들려면 '명도'·'신용회복지원'을
  // 좁혀야 하고 그러면 "명도일이 걱정이에요"가 죽는다.
  // 복지 카페(cafe1535) 실사용 제목 156개 점검 — 0건이 58%였고 오라우팅까지 있었다.
  // 주제 29종과 신청서 서식 5종이 이미 있는데(2ffb2a0) 거기로 가는 말이 없었다.
  // 특히 "급여" 두 글자가 임금체불 동의어에 있어 생계급여·의료급여·주거급여를 삼켰다 —
  // 기초생활 수급자가 노동청 임금체불 절차를 안내받는 오안내였다.
  // 상속 카페(whitemarbles) 실사용 제목 321개 점검 — 0건은 12%로 양호했지만 오라우팅이 있었다.
  // '상속[재산분할]'에서 이혼 재산분할 주제 키가 부분문자열로 걸려 +6을 받는다.
  // 게시판 '상속재산 분할청구' 40개 중 26개가 이혼 재산분할로 갔다.
  // 279주제 전수 점검(2026-09-08) — 18개가 자기 제목으로도 자기 주제에 닿지 못했다.
  // 대부분 "특칙이 있는 전용 주제가 일반 주제에 먹히는" 패턴이다. 외국인은 통보의무 면제·
  // 미등록 포함, 플랫폼 노무제공자는 전속성 요건 폐지 같은 특칙이 있어 일반 주제로 보내면
  // 그 정보를 통째로 놓친다. (5인 미만 사업장·프리랜서 근로자성과 같은 계열)
  // 상속 상담은 거의 사망 표현으로 시작하는데 데이터는 '사망'만 쓴다.
  // "별세"·"세상을 떠나"가 0건이었다. 그리고 "부모님 돌아가시고 빚이 더 많아요"가
  // 개인회생(본인 빚)으로 갔다 — 상속 빚은 상속포기·한정승인이고 둘 다 안 날부터
  // 3개월이다. 놓치면 빚을 그대로 떠안는다(민법 제1019조). 기한이 걸린 오라우팅이다.
  it("사망 표현으로 시작하는 상속 상담이 상속 절차로 간다 (9/8)", async () => {
    const 상속계열 = /안심상속_재산조회|상속포기|한정승인|상속등기|상속재산분할|사망후행정처리/;
    const 실패: string[] = [];
    for (const q of [
      "아버지가 돌아가셨어요",
      "남편이 먼저 세상을 떠난 지 4년 됐습니다",
      "할아버지께서 별세하셨습니다",
      "어머니 사망 후 절차",
      "돌아가신 아버지 재산 확인하려면",
    ]) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!상속계열.test(top)) 실패.push(`  ✗ ${top.padEnd(22)} ← ${q}`);
    }
    // 상속 빚은 개인회생이 아니라 상속포기·한정승인이어야 한다 (3개월)
    const t = await callText("search_topics", { query: "부모님 돌아가시고 빚이 더 많아요" });
    const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
    if (!/상속포기|한정승인/.test(top)) 실패.push(`  ✗ ${top} ← 부모님 돌아가시고 빚이 더 많아요 (상속포기·한정승인이어야 함)`);
    expect(실패, `\n${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // 교통사고 카페(uccch10) 실사용 제목 494개 점검 — 0건이 40%였다(2026-09-08).
  // "과실비율"은 있는데 "과실좀 봐주세요"·"제 과실이 있을까요" 같은 가장 흔한 형태가
  // 0건이었고, 사고 유형(횡단보도·중앙선·추돌)과 보상 용어(향후치료비·대인합의)도 비어 있었다.
  it("교통사고 실사용 표현이 제 주제로 간다 (9/8)", async () => {
    const 교통 = /교통사고후속절차|교통사고과실비율분쟁|교통사고손해배상청구|교통사고형사합의/;
    const 실패: string[] = [];
    for (const q of [
      "과실좀 봐주세요", "제 과실이 있을까요?", "100대0 사고인데",
      "신호등 없는 횡단보도 사고입니다", "중앙선침범 아닌가요", "불법주정차 사고",
      "택시랑 사고났는데", "교통사고 통원치료 얼마나 받아야 하나요", "교통사고 향후치료비 어떻게 계산해요",
      "대인합의 문의드려요",
    ]) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!교통.test(top)) 실패.push(`  ✗ ${top.padEnd(22)} ← ${q}`);
    }
    expect(실패, `\n10개 중 ${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // 사고·과실·치료 어휘를 넓혔으니 산재·의료·자전거·보험을 뺏지 않는지 본다.
  it("교통사고 동의어가 산재·의료·자전거를 뺏지 않는다 (9/8)", async () => {
    for (const [q, want] of [
      ["일하다 다쳤어요 산재", "산재요양급여신청"],
      ["의료사고 증거 확보하려면", "의료사고대응증거확보"],
      ["자전거 타다 사고났어요", "자전거킥보드사고"],
      ["전동킥보드 타다 사고", "자전거킥보드사고"],
      ["보험금을 안 준대요", "보험금부지급대응"],
      ["음주운전으로 면허 취소됐어요", "운전면허_행정처분구제"],
    ] as const) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe(want);
    }
  ...
  // 상속 세부 절차 — 재산조회·유류분·기여분·분할. 주제는 다 있는데 닿지 않았다.
  // "생전에 증여"로 넣었다가 "생전에 동생에게 토지를 모두 증여하신"에 붙지 않아
  // 한 어절로 바꿨다(두 어절 사이에 말이 끼는 문제. HOWTO 9번 참고).
  // 9/10: 사망·상속이 없는 생전 재산 문의를 상속으로 단정하지 않도록 문맥을 명시한다.
  it("상속 세부 절차가 제 주제로 간다 (9/8)", async () => {
    const 사례: [string, RegExp][] = [
      ["돌아가신 아버지 채무가 얼마나 있는지 어떻게 하면 알수 있나요?", /안심상속_재산조회/],
      ["돌아가신 분 빚이 얼마인지 확인하려면", /안심상속_재산조회/],
      ["돌아가신 어머니께서 생전에 동생에게 토지를 모두 증여하신 상태입니다", /유류분반환청구/],
      ["돌아가신 아버지가 생전에 다른 형제한테만 재산을 다 줬어요", /유류분반환청구/],
      ["돌아가신 할아버지 재산 저도 나눠 가지고 싶은대요", /상속재산분할협의|상속재산분할심판/],
      ["세 형제 중 혼자 아버지 부양하고 있었는데 상속재산 기여분을 인정받고 싶어요", /상속재산분할심판|상속재산분할협의/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!기대.test(top)) 실패.push(`  ✗ ${top.padEnd(22)} ← ${q.slice(0, 40)}`);
    }
    expect(실패, `\n${사례.length}개 중 ${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // 상속 빚 동의어가 본인 빚(개인회생·파산)이나 사망 관련 다른 분야를 뺏지 않는지 본다.
  it("상속 빚 동의어가 본인 채무·산재·교통사고를 뺏지 않는다 (9/8)", async () => {
    for (const [q, want] of [
      ["빚이 너무 많아서 개인회생 하고 싶어요", "개인회생"],
      ["카드빚 때문에 파산 신청하려고요", "개인파산면책"],
      ["산재로 사망했을 때 유족급여", "산재휴업장해유족급여"],
      ["교통사고로 사망했어요 손해배상", "교통사고손해배상청구"],
    ] as const) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe(want);
    }
  ...
  // "괴롭힘"(명사)만 있어 "직장 상사가 괴롭혀요"가 0건이었다(2026-09-08).
  // 잔금을 안 줘/줍니다와 같은 계열 — 명사형만 넣으면 활용형을 놓친다.
  it("직장내괴롭힘이 활용형으로도 닿는다 (9/8)", async () => {
    for (const q of ["직장 상사가 괴롭혀요", "팀장이 계속 괴롭히는데", "부장이 폭언을 해요", "직장에서 왕따 당하고 있어요"]) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe("직장내괴롭힘");
    }
  });

  // 처음에 따돌림·왕따·폭언을 단독으로 넣었더니 학교폭력·가정폭력을 뺏었다. 좁힌 뒤 확인한다.
  it("직장내괴롭힘 동의어가 학교폭력·모욕을 뺏지 않는다 (9/8)", async () => {
    for (const [q, want] of [
      ["학교에서 왕따 당해요", "학교폭력신고접수"],
      ["모르는 사람이 모욕했어요", "모욕"],
      ["상사가 월급을 안 줘요", "임금체불"],
    ] as const) {
      const t = await callText("search_topics", { query: q });
  ...
  // 분야를 넘는 오라우팅 셋 (2026-09-08). 세금 문의가 지급명령으로, 임대차의
  // "2년 더 살고 싶은데 나가래요"가 부당해고로 갔다 — '나가래'가 해고 동의어라서다.
  it("분야를 넘는 오라우팅 — 조세·임대차가 제 분야로 간다 (9/8)", async () => {
    const 사례: [string, RegExp][] = [
      ["세금이 잘못 나왔는데 이의신청", /조세이의신청|과세전적부심사|조세심판청구|경정청구/],
      ["세무서 처분에 불복하고 싶어요", /조세이의신청|과세전적부심사|조세심판청구|행정심판/],
      ["집주인이 전세금 올려달래요", /계약갱신차임증액분쟁/],
      ["계약갱신요구권 쓰고 싶어요", /계약갱신차임증액분쟁/],
      ["2년 더 살고 싶은데 나가래요", /계약갱신차임증액분쟁/],
      ["임대료를 5% 넘게 올려달래요", /계약갱신차임증액분쟁/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!기대.test(top)) 실패.push(`  ✗ ${top.padEnd(22)} ← ${q}`);
    }
    expect(실패, `\n${사례.length}개 중 ${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // 위 동의어가 노동·민사 쪽을 뺏지 않는지 함께 본다.
  it("조세·임대차 동의어가 노동·민사를 뺏지 않는다 (9/8)", async () => {
    for (const [q, want] of [
      ["회사에서 부당하게 해고당했어요", "부당해고"],
      ["갑자기 나가라고 합니다 회사에서", "부당해고"],
      ["집주인이 전세금을 안 돌려줘요", "전세보증금반환"],
      ["법원에서 지급명령이 왔어요", "지급명령받았을때"],
    ] as const) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe(want);
    }
  ...
  // 279주제 전수 자가진단 — 주제 자신의 제목을 넣어 자기 주제로 돌아오는지 본다.
  // 발화를 지어내지 않고 한 번에 전부 볼 수 있고, 4~5초면 끝난다.
  //
  // ★ 이 테스트가 존재하는 이유: 2026-09-08에 가정폭력 동의어로 "보호명령"을 넣었더니
  //   아동학대의 '피해아동보호명령'까지 삼켰는데, 그때 npm test 253개가 하나도 잡지 못했다.
  //   전수 진단에서만 잡혔다. 동의어를 넓히면 예상 못 한 주제를 뺏는다 — 그걸 여기서 막는다.
  //
  // 아래 목록은 "현재 알려진 실패"다. 새 이름이 여기 추가돼야 한다면 그건 회귀다.
  // 고쳐서 목록에서 빠지는 것은 환영 — 그때는 이 배열에서 지우면 된다.
  it("279주제 자가진단 — 알려진 실패 외에 새로 생기지 않는다", async () => {
    const 알려진실패 = new Set([
      "부동산하자담보", "상고", "상속재산분할협의", "쌍방폭행정당방위",
      "외국인근로자_임금체불", "재산조회", "전세권임차권말소", "전세보증보험이행청구",
      "조세이의신청", "풍수해_시민안전보험", "플랫폼특수고용_산재", "항소",
      "형사합의처벌불원",
    ]);
    const 새실패: string[] = [];
    for (const k of TOPIC_KEYS) {
      // 제목에서 괄호 설명·부제를 떼어 '사람이 부를 법한 이름'만 남긴다
      const 이름 = PROCEDURES[k].제목.replace(/[（(].*$/, "").replace(/—.*$/, "").trim();
      if (!이름) continue;
      const t = await callText("search_topics", { query: 이름 });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (top !== k && !알려진실패.has(k)) 새실패.push(`  ✗ ${k.padEnd(24)} → ${top.padEnd(22)} ("${이름.slice(0, 24)}")`);
    }
    expect(새실패, `\n새로 생긴 자가진단 실패 ${새실패.length}개\n${새실패.join("\n")}\n`).toEqual([]);
  });

  // 가정폭력 — 안전 직결이라 따로 본다. "남편이 때려요"는 걸리는데
  // "남편이 자꾸 때리는데"는 0건이었다. 동의어가 "남편이 때"라 사이에 말이 끼면 붙지 않는다
  // (2026-09-08. 스토킹에서 겪은 것과 같은 문제). 주제가 넷이나 있는데 닿지 못했다.
  it("가정폭력 발화가 0건으로 떨어지지 않는다 (9/8)", async () => {
    const 폭력계열 = /가정폭력|폭행|특수폭행|협박|스토킹|아동학대/;
    const 실패: string[] = [];
    for (const q of [
      "남편이 때려요",
      "남편이 자꾸 때리는데 어떻게 해야하나요",
      "집에서 폭력을 당하고 있어요",
      "남편한테 맞고 살아요",
      "아빠가 술만 마시면 때려요",
      "보호명령 신청하려면",
      ...
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!폭력계열.test(top)) 실패.push(`  ✗ ${top.padEnd(24)} ← ${q}`);
    }
    expect(실패, `\n7개 중 ${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // "보호명령" 동의어를 넣었더니 아동학대의 '피해아동보호명령'까지 삼켰다
  // (2026-09-08 전수 재점검에서 스스로 낸 회귀를 발견). 아동 신호가 있으면 아동 절차로 간다.
  it("아동학대 보호명령이 가정폭력 보호명령에 먹히지 않는다 (9/8)", async () => {
    for (const [q, want] of [
      ["피해아동보호명령 청구", /아동학대피해아동보호명령/],
      ["아동학대 신고하고 싶어요", /아동학대신고응급조치|아동학대피해아동보호명령/],
      ["보호명령 신청하려면", /가정폭력피해자보호명령/],
    ] as const) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      expect(want.test(top), `'${q}' → ${top}`).toBe(true);
    }
  });

  // 관계어·폭력 어휘를 넓혔으니 폭력이 아닌 가족 문의를 뺏지 않는지 함께 본다.
  it("폭력이 아닌 가족 문의는 가정폭력으로 가지 않는다 (9/8)", async () => {
    const 가정폭력 = /가정폭력/;
    for (const q of ["남편이 바람폈어요 이혼하고 싶어요", "이혼하고 재산분할 하려면", "남편한테 재산 상속받으려면", "남편이 자꾸 술을 마셔요"]) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      expect(가정폭력.test(top), `'${q}' 가 ${top} 로 갔다`).toBe(false);
    }
  });

  it("특칙이 있는 전용 주제가 일반 주제에 먹히지 않는다 (9/8)", async () => {
    const 사례: [string, RegExp][] = [
      ["미등록 외국인인데 일하다 다쳤어요", /외국인근로자_산업재해/],
      ["배달라이더인데 사고났어요", /플랫폼특수고용_산재/],
      ["대리운전 하다 다쳤어요", /플랫폼특수고용_산재/],
      ["범죄피해자 지원 받으려면", /범죄피해자_지원|범죄피해자_신변보호/],
      ["고용허가제 사업장 변경", /고용허가제_사업장변경/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
      if (!기대.test(top)) 실패.push(`  ✗ ${top.padEnd(24)} ← ${q.slice(0, 34)}`);
    }
    expect(실패, `\n${사례.length}개 중 ${실패.length}개 실패\n${실패.join("\n")}\n`).toEqual([]);
  });

  // 위 동의어는 '외국인'·직군 이름처럼 넓은 말이라 다른 주제를 뺏을 수 있다.
  it("특칙 동의어가 일반·인접 주제를 뺏지 않는다 (9/8)", async () => {
    for (const [q, want] of [
      ["월급을 두 달째 못 받고 있어요", "임금체불"],
      ["외국인등록증 발급 어디서 해요", "사증발급거부대응"],
      ["체류기간 연장하려면", "체류기간연장변경"],
      ["난민 신청하고 싶어요", "난민신청"],
      ["택배 물건이 파손됐어요", "택배분실파손"],
    ] as const) {
  ...
  it("상속재산분할이 이혼 재산분할로 가지 않는다 (9/8)", async () => {
    const 사례: [string, RegExp][] = [
      ["상속재산 분할청구 될까요?", /상속재산분할협의|상속재산분할심판/],
      ["상속재산분할협의서를 공증할 때요", /상속재산분할협의|상속재산분할심판/],
      ["상속재산분할 어떻게 해야할지 모르겠어요", /상속재산분할협의|상속재산분할심판/],
      ["상속재산 분할청구 기여분이요", /상속재산분할협의|상속재산분할심판/],
      ["상속재산분할소송 어떻게 하나요?", /상속재산분할협의|상속재산분할심판/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
  ...
  // 위 동의어가 이혼 재산분할을 뺏지 않는지 함께 본다 — '상속'이 없는 문장은 그대로 가야 한다.
  it("이혼 재산분할은 상속 쪽으로 넘어가지 않는다 (9/8)", async () => {
    for (const q of ["이혼하는데 재산분할 어떻게 해요", "재산분할 청구하고 싶어요", "이혼 재산분할 소송"]) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe("재산분할");
    }
  });

  it("복지 급여가 노동 임금체불로 끌려가지 않는다 (9/8)", async () => {
    const 사례: [string, RegExp][] = [
      ["생계급여", /기초생활보장_수급신청/],
      ["의료급여 궁금해요", /기초생활보장_수급신청|중증질환_산정특례/],
      ["주거급여 신청가능할까요", /주거급여_공공임대/],
      ["기초수급자 신청할때 통장거래내역", /기초생활보장_수급신청/],
      ["소득인정액 계산방법", /기초생활보장_수급신청/],
      ["조건부수급이 자활 해야되는건가요", /기초생활보장_수급신청/],
      ["긴급생계비 신청할때 통장에 돈 있으면 안되죠", /긴급복지지원/],
      ["한부모가정 지원받을 수 있나요", /한부모가족_지원/],
      ["한부모가정신청한지한달반", /한부모가족_지원/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
  ...
  // "급여"를 임금체불에서 뺐으니 노동 쪽이 깨지지 않았는지 함께 본다.
  it("급여 단독을 빼도 임금체불 경로는 유지된다 (9/8)", async () => {
    for (const q of ["급여를 못 받았어요", "급여가 안 들어왔어요", "회사에서 급여를 안 줍니다", "두 달째 급여 안 나와요"]) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe("임금체불");
    }
    // 인접 노동 주제도 제 갈 길을 간다
    for (const [q, want] of [["실업급여 신청하려는데", "실업급여신청"], ["육아휴직급여 언제 나와요", "육아휴직급여출산휴가급여"]] as const) {
      const t = await callText("search_topics", { query: q });
      expect((t.match(/- `([^`]+)`/) ?? [])[1], q).toBe(want);
    }
  });

  it("카페 후기·잡담 제목은 주제로 끌려가지 않는다 (9/7)", async () => {
    for (const q of [
      "우편물 100% 방어 도전해보겠습니다",
      "돈모으는데 소금이 필요함",
      "채무자의자세!",
      "불안해서 카페에 자주 들어오시는 분들께 드리고싶은 말이 있어요",
    ]) {
      const t = await callText("search_topics", { query: q });
      expect(t, `'${q}' 가 주제로 끌려갔다`).toContain("찾지 못했습니다");
    }
  });
