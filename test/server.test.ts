...
`).toEqual([]);
  ...
  it("교통사고 실사용 표현이 제 주제로 간다 (9/8)", async () => {
    ...
    for (const q of [
      ...
      "택시랑 사고났는데", "통원치료 얼마나 받아야 하나요", "향후치료비 어떻게 계산해요",
      "대인합의 문의드려요",
    ]) {
      const t = await callText("search_topics", { query: q });
      const top = (t.match(/- `([^`]+)`/) ?? [])[1] ?? "(0건)";
  ...
  it("상속 세부 절차가 제 주제로 간다 (9/8)", async () => {
    const 사례: [string, RegExp][] = [
      ["아버지 채무가 얼마나 있는지 어떻게 하면 알수 있나요?", /안심상속_재산조회/],
      ["돌아가신 분 빚이 얼마인지 확인하려면", /안심상속_재산조회/],
      ["어머니께서 생전에 동생에게 토지를 모두 증여하신 상태입니다", /유류분반환청구/],
      ["생전에 다른 형제한테만 재산을 다 줬어요", /유류분반환청구/],
      ["할아버지 재산 저도 나눠 가지고 싶은대요", /상속재산분할협의|상속재산분할심판/],
      ["세 형제 중 혼자 아버지 부양하고 있는데", /상속재산분할심판|상속재산분할협의/],
    ];
    const 실패: string[] = [];
    for (const [q, 기대] of 사례) {
      const t = await callText("search_topics", { query: q });
